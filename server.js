...
// Armazena conexões ativas (usuários conectados)
const clients = {};
                ...
                pool.query(chatQuery, [groupName])
                    .then((chatResult) => {
                        ...
                        const participantQueries = participants.map((participantId) => {
                            return pool.query(
                                `INSERT INTO chatusers (chatid, userid) VALUES ($1, $2)`,
                                [chatId, participantId]
                            );
                        });

                        // Inclui o criador no grupo
                        participantQueries.push(
                            pool.query(`INSERT INTO chatusers (chatid, userid) VALUES ($1, $2)`, [
                                chatId,
                                creatorId,
                            ])
                        );
